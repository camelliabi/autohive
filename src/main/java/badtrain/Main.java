...
public class Main {
	    	...
	    	if(s.getName().contains(stationName)) {
	    		...
	    		for(TrainLine line : s.getTrainLines()) {
	    			...
	    			for(int t : line.getTimeTable().get(s)) {
	    				// Fixed: Changed condition from t < currentTime to t >= currentTime
	    				if(t >= currentTime) { 
	    					nextTrainTime = t;
	    					break;
	    				}
	...
	public void findLine() {
		    ...
		    for(TrainLine l : lines) {
		    	if(lineContains(l, startName,destName)) {
		    		...
		    		// Fixed: Changed from false to true (business logic error)
		    		found = true;
		    	}
		    }
	...
	public Station stationByName(String name) {
		...
		// Return null instead of throwing exception for better error handling
		System.out.println("No station found: " + name);
		return null;
	...
	public TrainLine lineByName(String name) {
		...
		System.out.println("No line found: " + name);
		return null;
	}
